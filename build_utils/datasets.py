    ...
    def __init__(self,
        ...
        try:
            ...
            if os.path.isfile(path):  # file
                ...
                with open(path, "r") as f:
                    f = f.read().splitlines()
                    f = [
                        x.replace('./', '/root/cv/dataset/人脸/datasets/') for x
                        in f]
            else:
        ...
        for i, file in enumerate(pbar):
            ...
            if l.shape[0]:
                ...
                # print(l, '-----------------')
                # assert (l >= 0).all(), "negative labels: %s, l->" % file % l   超出范围
                # assert (l[:, 1:] <= 1).all(), "non-normalized or out of bounds coordinate labels: %s" % file 超过1...
                # l_negative = l[l >= 0]
                # print(l_negative)
