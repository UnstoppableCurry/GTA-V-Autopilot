...
def draw_text_4_openCV(img, box_to_display_str_map, box, left, right, top, bottom, color):
    try:
        font = ImageFont.truetype('arial.ttf', 20)
    except IOError:
        font = ImageFont.load_default()
    ...
    for display_str in box_to_display_str_map[box][::-1]:
        text_width, text_height = font.getsize(display_str)
        margin = np.ceil(0.05 * text_height)
        img = cv2.putText(img, display_str, (int(left + margin), int(text_bottom - text_height - margin)),
...
def darw_box_4_openCV(image, boxes, classes, scores, category_index, thresh=0.1, line_thickness=3):
    box_to_display_str_map = collections.defaultdict(list)
    box_to_color_map = collections.defaultdict(str)
    ...
    # for box in boxes:
        ...
        (left, right, top, bottom) = (xmin * 1, xmax * 1,
                                      ymin * 1, ymax * 1)
        ima = cv2.rectangle(ima, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (138, 43, 226), 3)
        ima = draw_text_4_openCV(ima, box_to_display_str_map, box, left, right, top, bottom,  color)
    return ima
